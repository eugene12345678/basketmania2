...
//     // Normalize the team choice input to lowercase for consistent comparison
//     const normalizedTeamChoice = teamChoice.toLowerCase();
  
//     // Check if the player is already in the selected team using their unique `id`
//     if (normalizedTeamChoice === 'alpha') {
//       const isPlayerInAlpha = myTeamAlpha.some(p => p.id === player.id);
//       if (isPlayerInAlpha) {
...
//     } else if (normalizedTeamChoice === 'beta') {
//       const isPlayerInBeta = myTeamBeta.some(p => p.id === player.id);
//       if (isPlayerInBeta) {
...
const App1 = () => {
  ...
  const handleRemovePlayer = (playerId, team) => {
    console.log(`Removing player with ID: ${playerId} from team: ${team}`);
    
    if (team === 'Alpha') {
      setMyTeamAlpha(prevTeam => {
        const newTeam = prevTeam.filter(player => player.id !== playerId); // Use player.id
        console.log('New Alpha team:', newTeam);
        return newTeam;
      });
    } else if (team === 'Beta') {
      setMyTeamBeta(prevTeam => {
        const newTeam = prevTeam.filter(player => player.id !== playerId); // Use player.id
        console.log('New Beta team:', newTeam);
        return newTeam;
      });
    }
  };
